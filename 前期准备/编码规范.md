...
包名应该和目录名一致，且必须简短、有意义。

包名必须采用全小写的命名风格，不要在其中使用任何下划线或者混合大小写，如：
...
注释中如果出现中英文，那么中英文字符之间应当严格使用空格进行分割：

```
// 这是一个 comment 注释。
```

对于单行注释而言，如果注释排在代码后面应当使用 2 个空格进行分割，并且注释 // 后面应该有 1 个空格：
...
每个自定义的接口或结构体都应该具有注释说明，其中的每个字段也应当有注释介绍：
...
代码缩进使用 tab 进行，建议安装 gofmt 工具进行自动格式化。
...
不要使用 \_ 忽略所有的错误返回，函数返回值应当全部接收并判定其 err 是否 为 nil。
