...
Golang 内置 new 函数可以为指针变量开辟内存空间，如果我们只声明了一个指针变量（空指针），但是未对该指针变量进行赋值时可以使用 new 函数来开辟一片内存空间。
...
make 函数作为切片、映射、通道这 3 种引用类型的初始化函数存在。

在本章节[引用初始化](/数据类型/指针类型?id=引用初始化)中已经例举了，这里不再进行演示。
