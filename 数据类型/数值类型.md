...
注意！len 函数可根据不同平台动态获取动态整形长度，但是在涉及到二进制传输、读写文件结构描述时，需要谨慎使用 uint 和 int。
...
在计算机领域里则用 e 表示 10 的幂，下表是科学计数法在数学领域和计算机领域中不同表现形式：
...
对于极大或极小数，请采取科学计数法记录。从代码观感、程序运行角度来说都是十分不错的选择。
